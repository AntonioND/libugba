...
static int gui_pos_is_inside_rect(int xm, int ym, int x, int w, int y, int h)
{
...
static int gui_inputwindow_send_event(gui_inputwindow *win, SDL_Event *e)
{
    if (win == NULL)
        return 0;
...
static int gui_menu_send_event(gui_menu *menu, SDL_Event *e)
{
    if (menu == NULL)
        return 0;
    ...
    while (1)
    {
        ...
        if (gui_pos_is_inside_rect(e->button.x, e->button.y,
...
static int gui_get_first_window_enabled(gui_global *gui)
{
    if (gui == NULL)
        return -1;

    gui_element **gui_elements = gui->elements;
...
static int gui_get_first_messagebox_enabled(gui_global *gui)
{
    if (gui == NULL)
        return -1;
...
int gui_send_event_element(gui_element **completegui, gui_element *gui,
                           SDL_Event *e)
{
    ...
    else if (gui->element_type == GUI_TYPE_BUTTON)
    {
        ...
        {
            if (gui_pos_is_inside_rect(e->motion.x, e->motion.y,
                                       gui->x, gui->w, gui->y, gui->h))
            {
                gui->info.button.is_pressed = 0;
                return 1;
            ...
            if (e->type == SDL_MOUSEBUTTONDOWN)
            {
                if (e->button.button == SDL_BUTTON_LEFT)
                {
                    if (gui_pos_is_inside_rect(e->button.x, e->button.y,
                                               gui->x, gui->w, gui->y, gui->h))
                    {
                        gui_clear_radiobuttons(completegui,
    ...
    else if (gui->element_type == GUI_TYPE_WINDOW)
    {
        ...
        gui_global *relativegui = gui->info.window.gui;
        return GUI_SendEvent(relativegui, e);
    }
    ...
    else if (gui->element_type == GUI_TYPE_SCROLLABLETEXTWINDOW)
    {
        ...
        if (e->type == SDL_MOUSEBUTTONDOWN)
        {
            if (e->button.button == SDL_BUTTON_LEFT)
            {
                if (gui_pos_is_inside_rect(e->button.x, e->button.y, gui->x,
                                           gui->w, gui->y, gui->h) == 0)
                {
...
int GUI_SendEvent(gui_global *gui, SDL_Event *e)
{
    // Highest priority = input window
    if (GUI_InputWindowIsEnabled(gui->inputwindow))
    {
        return gui_inputwindow_send_event(gui->inputwindow, e);
    }

    // Very high priority = menu
    if (gui_menu_send_event(gui->menu, e))
        return 1;

    gui_element **elements = gui->elements;
    if (elements == NULL)
        return 0;

    // High priority = message box
    int messagebox_enabled = gui_get_first_messagebox_enabled(gui);
    if (messagebox_enabled >= 0)
    {
        return gui_send_event_element(gui->elements,
                                      elements[messagebox_enabled], e);
    }

    // Normal priority = other windows
    int window_enabled = gui_get_first_window_enabled(gui);
    if (window_enabled >= 0)
    {
        return gui_send_event_element(gui->elements,
    ...
    while (*elements != NULL)
    {
        if (gui_send_event_element(gui->elements, *elements++, e))
            return 1;
    }
